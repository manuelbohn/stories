...
print(data%>%group_by(testlanguage)%>%summarise(n = n_distinct(child)),n=43)
#    testlanguage     n
#    <chr>        <int>
#  1 afr            110
#  2 aka             39
...
# Corrected and valid Newick tree grouped by language families
newick <- "(
  (eng,nld,ger,afr,dan,swe),                # Germanic
  (ita,esp,cat,cor),                        # Romance
  (rus,ukr,hrv,slk,pol),                    # Slavic
  (gle,ghc),                                # Celtic
  (hin,urd),                                # Indo-Aryan
  (kan,gon,hlb),                            # Dravidian
  (per,tgk,sgh),                            # Iranian
  (fin,est,sah),                            # Uralic
  (tur,kaz,uzb,uig),                        # Turkic
  (cmn,yue,iii),                            # Sino-Tibetan
  (kor),                                    # Koreanic
  (tgl,msa),                                # Austronesian
  (heb,ary),                                # Afro-Asiatic
  (aka,lug,xho),                            # Niger-Congo
  (bi,bis),                                 # Creoles
  (geo),                                    # Kartvelian
  (ell,cg),                                 # Hellenic
  (ber)                                     # Berber
...
lang_labels <- c(
  per = "Persian", hrv = "Croatian", rus = "Russian", est = "Estonian",
  fin = "Finnish", swe = "Swedish", geo = "Georgian", tur = "Turkish",
  aka = "Akan", urd = "Urdu", hin = "Hindi", kan = "Kannada", gon = "Gondi",
  hlb = "Halbi", gle = "Irish", heb = "Hebrew", ita = "Italian", uzb = "Uzbek",
  bi = "Bislama", nld = "Dutch", tgl = "Tagalog", pol = "Polish", eng = "English",
  slk = "Slovak", sgh = "Shughni", lug = "Luganda", sah = "Sakha",
  cor = "Coriglianese", dan = "Danish", bis = "Bislama", iii = "Sichuan Yi",
  cat = "Catalan", ger = "German", kaz = "Kazakh", uig = "Uighur",
  lt = "Lithuanian", tgk = "Tajik", msa = "Malay", esp = "Spanish",
  kor = "Korean", cg = "Cypriot Greek", yue = "Yue Chinese", ell = "Greek",
  ber = "Berber languages", ary = "Moroccan Arabic", xho = "Xhosa",
  afr = "Afrikaans", cmn = "Mandarin Chinese", ghc = "Hiberno-Scottish Gaelic",
  ukr = "Ukrainian"
)

# Apply language name mapping to tip labels
tree$tip.label <- lang_labels[tree$tip.label]

# Plot tree (rectangular layout for better label readability)
ggtree(tree, layout = "rectangular") +
  geom_tiplab(size = 3, align = TRUE, linetype = "dotted") +
  ggtitle("Phylogenetic Tree of Selected Languages") +
  theme_tree2() +
...
```
```{r}
ggsave("../visuals/map.png", width = 7, height = 4, scale = 1)
```
